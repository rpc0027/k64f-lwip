/** @file lcd.c
*
* @brief Use a LCD display of 2 lines of 16 characters via I2C bus.
*
*/
...
* @param[in] charsize Size of the characters.
* @return The address of the initialized LCD.
*/
uint8_t LCD_Init(uint8_t lcd_addr, uint8_t lcd_cols, uint8_t lcd_rows, uint8_t charsize)
{
	...
	// Slave selection.
	//
	select = select_slave(g_lcd_addr);
...
//	while(II2C_CheckBus()!=II2C_IDLE) wait_Waitus(10);
	//wait_Waitms(1);
